...
function 本の姿(id, x){
  ...
  return {
      ...
      /* ⚠️⚠️ 「絶版」と出すのは、**確かめた本だけ**（2026-09-26 決定 C）。
            出版社が絶版と書いている・出版社がもう無い、など根拠のあるもの。books.outOfPrint = { note, checkedAt }。
            Amazon に新品が無いだけの本は「品切れ」のまま（出版社は「品切・重版未定」とし、絶版とは言わないことが多い）。
            中の status はどちらも "絶版"（「復刊を願う」などの分かれ目は変えない） */
      絶版の根拠: x.status === "絶版" && x.outOfPrint?.note ? x.outOfPrint.note : null,
      絶版の日: x.outOfPrint?.checkedAt || null,
      色: 題から色(x.title),
      受取: (x.to || []).map(id=>{
        const e = 主体表.get(id);
...
/* 既にある：申請の本がもう棚にあった（2026-09-26）。見送りとは分ける（断ったのではない）。book に棚の本が入る */
export const 申請の状態 = { 確認中:"確認中", 並んだ:"棚に並びました", 既にある:"もう棚にありました", 見送り:"見送り", 処理済み:"処理済み" };
...
/* ============================================================
   小道具
   ============================================================ */
/* 画面に出す状態の名。⚠️ 「絶版」は確かめた本だけ（上の 絶版の根拠）。ほかは「品切れ」 */
export const 状態の名 = b => b.状態 !== "絶版" ? "流通中" : b.絶版の根拠 ? "絶版" : "品切れ";
