...
async function 頁_一覧(){
  ...
  /* ⚠️ 前は一番下に「さがしている本がありませんか」の節があったが、2026-09-26 に外した。
        頭の 棚の育ち() に同じボタンがあり、検索が空振りしたときにも出るので、3か所に重なっていた */
}
...
window.設定をひらく = (はじめて = false)=>{
  const 決めてある = 土台.名乗り表.has(土台.私?.uid);
  const 私の = 土台.私の印();
  /* ⚠️⚠️ **まだ名前を決めていない人には、Google の名前を入れておかない。**
        入れておくと、そのまま「これにする」を押しただけで本名が他の人の画面に出る（2026-09-26）。
        しるしの字も Google の名前の1字目になるので、同じく入れない */
  設定中 = { 名: 決めてある ? 土台.私の名() : "", 印: 決めてある ? 私の.印 : "",
            色:私の.色, 顔: 決めてある ? 私の.顔 : null,
            公開:土台.公開か(土台.私?.uid), 送信中:false, 支度中:false, はじめて };
  設定描く();
};

/* まだ名前を決めていない人が入ったら、名前を決める窓を出す（2026-09-26 決定：A）。
   ⚠️ users/{uid} は、ここで「これにする」を押したときに初めてできる。入っただけではできない。
      前は窓を自分で開くまで作られず、4人中3人が名前を決めないまま（他の人の画面では「読者」）だった。
   ⚠️ 1つのタブで1度だけ。閉じた人に、読み込み直すたびには出さない */
function 名前を決めてもらう(){
  const uid = 土台.私?.uid;
  if(!uid || !土台.名乗りを読めた || 土台.名乗り表.has(uid)) return;
  const 鍵 = "名前を聞いた:" + uid;
  try{ if(sessionStorage.getItem(鍵)) return; sessionStorage.setItem(鍵, "1"); }catch(e){}
  設定をひらく(true);
}
...
function 設定描く(){
  ...
  窓を出す(設定中.はじめて ? "はじめに、名前を決めてください" : "設定", `
        ${設定中.はじめて ? `<p class="節の注" style="margin:0 0 18px">
          本返しでは、Google の名前は出しません。読者の声や番付に出す名前を決めてください。
          本名でなくてかまいません。あとからマイページで変えられます。</p>` : ""}
        <div class="見本の列">
          ${見本}
          <div>
        ...
        <p class="節の注">
          読者の声や番付に、この名前と印が出ます。${決めてある ? ""
            : "<br>まだ決めていないので、ほかの人の画面では「読者」と出ています。"}
